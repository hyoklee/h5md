...
class DataValidator:
    ...
    def _check_numeric_constraints(self, data: np.ndarray, rule: ValidationRule) -> List[str]:
        issues = []
    ...
    def validate_dataset(self, dataset: h5py.Dataset, rule: ValidationRule) -> ValidationResult:
        issues = []
        
        # Check dtype
        if rule.dtype and str(dataset.dtype) != rule.dtype:
            issues.append(f"Incorrect dtype: expected {rule.dtype}, got {dataset.dtype}")
        
        # Check shape
        if rule.shape:
            if len(dataset.shape) != len(rule.shape):
                issues.append(f"Incorrect number of dimensions: expected {len(rule.shape)}, got {len(dataset.shape)}")
            else:
                for expected, actual in zip(rule.shape, dataset.shape):
                    if expected is not None and expected != actual:
                        issues.append(f"Incorrect dimension size: expected {expected}, got {actual}")
